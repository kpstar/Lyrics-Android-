...
import com.example.kpstar.lyricsapp.Lyrics.LRC.Offset;
...
public class LRCParser {
     ...
     * @return LRC
     */
    public static LRC parseFromFile(FileInputStream in) {
        // by zhangjb add
...
//        File destFile = null;
    ...
    public static void isValidLRCFile(File path) {
        ...
        if (!path.canRead()) {
            throw new IllegalArgumentException("lrc file [" + path.getPath() + "] can not be read.");
        }
        if (!path.getName().toLowerCase().endsWith(LyricConstants.EXTENSION)) {
            throw new IllegalArgumentException("[" + path.getName() + "] is not a valid lrc file.");
        }
    }
     ...
     * @throws IOException
     */
    public static File toUtf(String type, File sourceFile) throws IOException {
        if (type == LyricConstants.TEXT_UTF_ENCODING) {
            return sourceFile;
        } else {
            ...
            br.close();
            File dir = new File(StringConstant.CURRENT_PATH, StringConstant.LRC_DIRECTORY
                    + File.separator);
            if (!dir.exists()) {
                dir.mkdirs();
            }
            File tempFile = new File(dir, LyricConstants.TEMP_LRC_FILE);
            Writer write = new OutputStreamWriter(new FileOutputStream(tempFile),
                    LyricConstants.TEXT_UTF_ENCODING);
            write.write(new String(utfContent.toString().getBytes(LyricConstants.TEXT_UTF_ENCODING)));
            write.close();
            return tempFile;
        }
    }
